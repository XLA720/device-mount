...
screen() {
    ...
    blkid > /home/device.txt
    python .device.py device
    devicename=$(python .device.py name)
    maxnumber=$(python .device.py number)
    echo "================================================"
    ...
    while true; do
        if [[ -z ${dev:-} ]]; then
            ...
            while true; do
                userchoose=$(python .device.py rangein ${maxnumber} ${dev})
...
mountloop() {
    ...
    mountroot=($(python .device.py path ${dir} ${userchoose}))
    for ((i=0; i<${#mountroot[@]}; i+=2)); do
        ...
        sudo mount ${mdev} ${mdir} && echo "device ${mdev} mount Complete." || { echo "Error: mount"; exit 1; }
    done
}
...
exe         ==> 判断程序，python调用格式：「自定义名称」=($(python .device.py 「调用函数」 「传入参数」))
